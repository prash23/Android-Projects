...
public class TestComplex extends AppCompatActivity {
    private static final int REQUEST_CODE = 1234;
    private Toolbar mToolbar;
    public String angleABC,angleACD,side,units;
    public TestComplexOutput complexOutput;
    private int flag = 0;
    public Intent it;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.testcomplexoutput);
        setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_PORTRAIT);
        mToolbar = (Toolbar) findViewById(R.id.toolbar);
        setSupportActionBar(mToolbar);
        getSupportActionBar().setTitle("");
        TextView title = (TextView) mToolbar.findViewById(R.id.actionbar_textview);
        title.setText("COMPLEX PROBLEM");
        getSupportActionBar().setDisplayHomeAsUpEnabled(true);

      it = getIntent();

        if(getIntent().hasExtra("angleABC"))
        {
            angleABC = it.getStringExtra("angleABC");
            angleACD = it.getStringExtra("angleACD");
            side = it.getStringExtra("side");
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        switch (item.getItemId()) {
            case R.id.save:
                verifyStoragePermissions(TestComplex.this);
                List<String> saveValues = new ArrayList<>();
                saveValues.add(side);
                saveValues.add(angleABC);
                saveValues.add(angleACD);
    ...
    public void alertFormElements(final List<String> saveValues) {
        ...
        new AlertDialog.Builder(TestComplex.this).setView(formElementsView)
    ...
    private boolean createDirectoryAndSaveFile(String s1,List<String> data) throws IOException {
        ...
        {
            File directory = new File(Environment.getExternalStorageDirectory().getAbsolutePath() + "/GeoHelper/Complex");
            directory.mkdirs();
        ...
        {
            File directory = new File(getExternalFilesDir("/GeoHelper/Complex"),s1+".txt");
            try {
    ...
    protected void onResume() {
        complexOutput = (TestComplexOutput) findViewById(R.id.outputView);
        complexOutput.angleABC = Double.parseDouble(angleABC);
        complexOutput.angleACD = Double.parseDouble(angleACD);
        complexOutput.sideBC = Double.parseDouble(side);
        complexOutput.units=units;
    ...
    public void finish()
    {
        ...
        intent.putExtra("angleABC",angleABC);
        intent.putExtra("angleACD",angleACD);
